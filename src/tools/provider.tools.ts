...
export class ProviderTools {
  ...
  async searchProviders(args: {
    ...
    try {
      ...
      const result = await this.fhirService.search('Practitioner', searchParams, args.limit || 20);
      const providers = result.resources.map(practitioner => FHIRParser.parsePractitioner(practitioner));
   ...
   */
  async getProviderDetails(args: { providerId: string }): Promise<ParsedPractitioner> {
    try {
      if (!args.providerId) {
        throw ErrorHandler.createValidationError('Provider ID is required');
      }

      const practitioner = await this.fhirService.getResource('Practitioner', args.providerId);
  ...
  async searchLocations(args: {
    ...
    try {
      ...
      const result = await this.fhirService.search('Location', searchParams, args.limit || 20);
  ...
  async getLocationDetails(args: { locationId: string }): Promise<{
    ...
    try {
      ...
      const location = await this.fhirService.getResource('Location', args.locationId);
      const address = location.address?.[0];
      const telecom = location.telecom || [];
      const phone = telecom.find(t => t.system === 'phone')?.value;
      const email = telecom.find(t => t.system === 'email')?.value;
